## Tema personalizado
jade utiliza la metodología BEM en CSS con la finalidad de que puedas sobrescribir los estilos fácilmente. Pero, si necesita remplazar estilos a gran escala, por ejemplo, cambiar el color del tema de azul a naranja o verde, quizás reemplazarlos uno a uno no sea lo más adecuado, para ello hay 3 maneras de modificar los estilos.

### Cambiando el color del tema
Si lo que se busca es cambiar el color del tema de jade, se recomienda utilizar el [sitio de visualización de temas](https://elementui.github.io/theme-chalk-preview/#/en-US). jade utiliza un color azul brillante y amigable como tema principal. Al cambiarlo, puede hacer que jade este más conectado visualmente a proyectos específicos.
...
`theme-chalk` esta escrito en SCSS. Si su proyecto también utiliza SCSS, puede cambiar las variables de estilos de jade. Para ello, solo necesita crear un nuevo archivo de estilos, por ejemplo, `jade-variables.scss`:
...
Entonces, en el archivo principal del proyecto, importe este archivo de estilos en lugar de los estilos de jade:
```JS
import Vue from 'vue'
...
:::tip
Nota es necesario sobreescribir la ruta de la fuente por una ruta relativa de las fuentes de jade.
:::
...
En el archivo `jade-variables.scss` podrá encontrar todas las variables que utiliza jade para definir los estilos y estos están definidos en SCSS. Aquí un ejemplo:
```css
...
Solo debe modificar el archivo `jade-variables.scss`, por ejemplo, para cambiar el color del tema a rojo:
```CSS
$--color-primary: red;
```
...
Si no esta familiarizado con `babel-plugin-component`, por favor diríjase a la documentación sobre <a href="./#/en-US/component/quickstart">Como Iniciar</a>. Para más detalles, consulte el [repositorio del proyecto](https://github.com/ElementUI/jade-theme) de `jade-theme`.